...
app.get(['/oauth/callback', '/api/oauth/callback'], async (req, res) => {
  ...
  try {
    ...
    const url = `https://dir.engageautomations.com/?installation_id=${id}&welcome=true`;
    console.log('Redirecting to:', url);
    res.redirect(url);
    
  } catch (e) {
    console.error('OAuth error:', e.response?.data || e.message);
    const url = `https://dir.engageautomations.com/?error=oauth_failed&timestamp=${Date.now()}`;    
    res.redirect(url);
  }
});
